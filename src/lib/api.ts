...
import type {
  ...
  PaymentRow, ProfileRow, SiteContent, NotificationRow, SecretRow,
} from '@/types';
...
export async function resendDealTerms(
  dealId: string,
  newTotalPrice?: number,
  newDownPayment?: number,
) {
  const { data, error } = await supabase.rpc('resend_deal_terms', {
    p_deal_id: dealId,
    p_new_total_price: newTotalPrice ?? null,
...
export async function nudgeBuyer(dealId: string) {
  const { data, error } = await supabase.rpc('nudge_buyer', {
    p_deal_id: dealId,
  });
  if (error) throw error;
  return data as {
...
export async function updateStaffAvatar(userId: string, avatarUrl: string) {
  const { error } = await supabase.rpc('update_staff_avatar', {
    p_user_id: userId,
...
export async function fetchAppSecrets(): Promise<SecretRow[]> {
  const { data, error } = await supabase.rpc('get_app_secrets');
  if (error) throw error;
  return data ?? [];
}

export async function upsertAppSecret(key: string, value: string): Promise<SecretRow> {
  const { data, error } = await supabase.rpc('upsert_app_secret', {
    p_key: key,
    p_value: value,
  });
  if (error) throw error;
